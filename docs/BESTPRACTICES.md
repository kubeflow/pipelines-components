...
- Aim for fast smoke coverage (import checks, signature validation, lightweight
  ...
  CI.
- Run `python .github/scripts/run_tests.py <path-to-component>` locally to execute only
  the suites you are touching. The helper automatically enforces the
  `pytest-timeout` guardrail used in CI.
...
- The CI workflow imports each module by running
  `python .github/scripts/validate_examples.py <paths>` for every changed component or
  pipeline and compiles the matching sample pipelines using `kfp.compiler`. Run
  the same command locally (with or without explicit paths) before submitting a
  PR to catch syntax or dependency regressions early.
