...
class TestImportGuardConfig:
    ...
    def test_from_path_nonexistent(self):
        ...
        config = ImportGuardConfig.from_path(Path("/nonexistent/file.json"))
        assert config.module_allowlist == set()
        assert config.path_scoped_allowlist == {}
...
class TestDiscoverPythonFiles:
    ...
    def test_restrict_to_directories(self):
        """Test restricting file discovery to specific directories."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir)
            components_dir = tmpdir_path / "components"
