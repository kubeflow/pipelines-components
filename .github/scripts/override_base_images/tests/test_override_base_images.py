...
from override_base_images import override_base_images, override_file_images
...
class TestOverrideFileImages:
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path):
        ...
        was_modified, new_content = override_file_images(
            py_file, COMMIT_SHA, IMAGE_PREFIX, dry_run=True
        )
    ...
    def test_no_modification_when_no_match(self, tmp_path: Path):
        ...
        was_modified, new_content = override_file_images(
            py_file, COMMIT_SHA, IMAGE_PREFIX
        )
...
class TestOverrideBaseImages:
    ...
    def test_modifies_files_in_directory(self, tmp_path: Path):
        ...
        (components / "comp.py").write_text(
            f'base_image="{IMAGE_PREFIX}-test:main"'
        )

        modified = override_base_images(
            [str(components)], COMMIT_SHA, IMAGE_PREFIX, verbose=False
        )
    ...
    def test_dry_run_returns_files_but_no_changes(self, tmp_path: Path):
        ...
        modified = override_base_images(
            [str(components)], COMMIT_SHA, IMAGE_PREFIX, dry_run=True, verbose=False
        )
    ...
    def test_scans_subdirectories(self, tmp_path: Path):
        ...
        (subdir / "component.py").write_text(
            f'base_image="{IMAGE_PREFIX}-training:main"'
        )
    ...
    def test_handles_nonexistent_directory(self, tmp_path: Path):
        modified = override_base_images(
            [str(tmp_path / "nonexistent")], COMMIT_SHA, IMAGE_PREFIX, verbose=False
        )
    ...
    def test_multiple_directories(self, tmp_path: Path):
        ...
        modified = override_base_images(
            [str(components), str(pipelines)], COMMIT_SHA, IMAGE_PREFIX, verbose=False
        )
