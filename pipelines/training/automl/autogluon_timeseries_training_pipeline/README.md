...
notebook). Refits run **one at a time** so only one pod mounts the pipeline workspace PVC; the workspace uses **ReadWriteOnce**, and higher parallelism causes **Multi-Attach** errors on typical block storage.
