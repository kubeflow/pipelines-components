...
from scripts.lib.tags import check_base_image_tags, check_main_tag, find_base_image_references
...
class TestFindBaseImageReferences:
    ...
    def test_finds_base_image_with_prefix(self, tmp_path: Path):
        """Finds a single base_image reference when it matches the configured prefix."""
...
"""
    ...
    def test_finds_multiple_references(self, tmp_path: Path):
        """Finds multiple base_image references within the same file."""
        py_file = tmp_path / "component.py"
        py_file.write_text(
...
"""
    ...
    def test_ignores_non_matching_base_image(self, tmp_path: Path):
        """Ignores base_image references that don't match the configured prefix."""
        py_file = tmp_path / "component.py"
        py_file.write_text(
            """from kfp import dsl
...
class TestCheckMainTag:
    ...
    def test_valid_main_tag(self):
        """Accepts references tagged with :main."""
    ...
    def test_valid_with_single_quotes(self):
        """Accepts :main when base_image is single-quoted."""
        line = f"base_image='{IMAGE_PREFIX}-example:main'"
    ...
    def test_valid_with_spaces(self):
        """Accepts :main when whitespace surrounds the assignment operator."""
...
class TestCheckBaseImageTags:
    ...
    def test_all_valid_returns_true(self, tmp_path: Path):
        """Returns all_valid=True when all references are tagged with :main."""
        components = tmp_path / "components"
        components.mkdir()
        (components / "comp.py").write_text(f'@dsl.component(base_image="{IMAGE_PREFIX}-test:main")\ndef comp(): pass')

        all_valid, results = check_base_image_tags([str(components)], IMAGE_PREFIX)
    ...
    def test_invalid_tag_returns_false(self, tmp_path: Path):
        """Returns all_valid=False when a reference uses a disallowed tag."""
        components = tmp_path / "components"
        components.mkdir()
    ...
    def test_mixed_valid_and_invalid(self, tmp_path: Path):
        """Reports mixed valid/invalid results when scanning multiple files."""
        components = tmp_path / "components"
        components.mkdir()
    ...
    def test_no_references_is_valid(self, tmp_path: Path):
        """Treats directories with no matching references as valid."""
        components = tmp_path / "components"
        components.mkdir()
        (components / "comp.py").write_text('@dsl.component(base_image="python:3.11")\ndef comp(): pass')
    ...
    def test_nonexistent_directory_is_valid(self, tmp_path: Path):
        """Treats nonexistent directories as valid (no references found)."""
        all_valid, results = check_base_image_tags([str(tmp_path / "nonexistent")], IMAGE_PREFIX)

        assert all_valid is True
        assert len(results) == 0

    def test_scans_subdirectories(self, tmp_path: Path):
        """Finds references in nested subdirectories."""
        components = tmp_path / "components"
    ...
    def test_multiple_directories(self, tmp_path: Path):
        """Scans multiple directories and aggregates results."""
        components = tmp_path / "components"
        components.mkdir()
        ...
        all_valid, results = check_base_image_tags([str(components), str(pipelines)], IMAGE_PREFIX)
