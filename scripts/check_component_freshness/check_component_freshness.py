...
def scan_repo(repo_path: Path) -> dict:
    ...
    for subdir in ["components", "pipelines"]:
        scan_dir = repo_path / subdir
        if not scan_dir.exists():
            continue
        for metadata_file in scan_dir.rglob("metadata.yaml"):
            try:
                metadata = yaml.safe_load(metadata_file.read_text())
                if not metadata or "lastVerified" not in metadata:
                    print(f"Warning: Missing lastVerified in {metadata_file}", file=sys.stderr)
                    continue
                ...
                results[category].append(
                    {
                        ...
                        "path": str(metadata_file.relative_to(repo_path).parent),
