...
def test_category_index_generation():
    ...
    subprocess.run(
        ["uv", "run", "python", "-m", "scripts.generate_readme",
         "--component", str(TEST_DATA_DIR / "components/basic/simple_component"),
         "--overwrite"],
        capture_output=True,
        cwd=Path(__file__).parent.parent.parent.parent
    )
    subprocess.run(
        ["uv", "run", "python", "-m", "scripts.generate_readme",
         "--component", str(TEST_DATA_DIR / "components/advanced/multiline_overview"),
         "--overwrite"],
        capture_output=True,
        cwd=Path(__file__).parent.parent.parent.parent
    )
    subprocess.run(
        ["uv", "run", "python", "-m", "scripts.generate_readme",
         "--pipeline", str(TEST_DATA_DIR / "pipelines/basic/simple_pipeline"),
         "--overwrite"],
        capture_output=True,
        cwd=Path(__file__).parent.parent.parent.parent
    )
