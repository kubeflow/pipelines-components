...
class TestCategoryIndexGenerator:
    ...
    def test_init_pipeline_category(self, tmp_path):
        ...
        assert generator.category_dir == category_dir
        assert not generator.is_component
    ...
    def test_extract_item_info_multiline_overview(
        self, tmp_path, component_multiline_overview, sample_component_metadata
    ):
        """Test that only first line of overview is used in index."""
        category_dir = tmp_path / "components" / "dev"
        category_dir.mkdir(parents=True)

        item_dir = create_component_dir(
            category_dir,
            "test_comp",
            component_multiline_overview,
...
tier: core
"""
        create_component_dir(category_dir, "test_comp", sample_component_file, metadata_content)
