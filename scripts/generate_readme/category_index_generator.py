...
class _BaseIndexGenerator:
    ...
    def _find_asset_dirs(self) -> List[Path]:
        ...
        Skips directories starting with '__' and directories named 'shared'.
        ...
        for subdir in self.directory.iterdir():
            if subdir.is_dir() and not subdir.name.startswith("__") and subdir.name != "shared":
...
class CategoryIndexGenerator(_BaseIndexGenerator):
    ...
    def _is_subcategory(self, subdir: Path) -> bool:
        ...
        for child in subdir.iterdir():
            if child.is_dir() and not child.name.startswith("__") and child.name != "shared":
