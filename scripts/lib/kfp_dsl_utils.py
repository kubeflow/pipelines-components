...
def find_decorated_functions(module: Any, decorator_type: str) -> list[tuple[str, Any]]:
    ...
    Raises:
        ValueError: If decorator_type is not 'component' or 'pipeline'.
    """
    if decorator_type not in ("component", "pipeline"):
        raise ValueError(f"decorator_type must be 'component' or 'pipeline', got '{decorator_type}'")
