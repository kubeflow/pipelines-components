...
_COMPONENT_DECORATORS = {"component", "container_component", "notebook_component"}
_PIPELINE_DECORATORS = {"pipeline"}
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            for decorator in node.decorator_list:
                ...
                if decorator_name is not None and decorator_name in _COMPONENT_DECORATORS:
                    result["components"].append(node.name)
                    break
                if decorator_name is not None and decorator_name in _PIPELINE_DECORATORS:
