...
from ..discovery import (
    _build_asset_dict_from_repo_path,
...
class TestBuildAssetDictFromRepoPath:
    """Tests for _build_asset_dict_from_repo_path()."""

    def test_direct_component_path(self, tmp_path: Path):
        """Parse a 3-part path: category/name/component.py."""
        comp_file = _make_component(tmp_path, "training", "my_trainer")

        result = _build_asset_dict_from_repo_path(tmp_path, "components", comp_file, "component.py")

        assert result["category"] == "training"
        assert result["name"] == "my_trainer"
        assert result["subcategory"] is None
        assert result["path"] == comp_file

    def test_subcategory_component_path(self, tmp_path: Path):
        """Parse a 4-part path: category/subcategory/name/component.py."""
    ...
    def test_direct_pipeline_path(self, tmp_path: Path):
        """Parse a 3-part pipeline path."""
        pipe_file = _make_pipeline(tmp_path, "training", "my_pipeline")

        result = _build_asset_dict_from_repo_path(tmp_path, "pipelines", pipe_file, "pipeline.py")

        assert result["category"] == "training"
        assert result["name"] == "my_pipeline"
        assert result["subcategory"] is None

    def test_subcategory_pipeline_path(self, tmp_path: Path):
        """Parse a 4-part pipeline path."""
    ...
    def test_raises_on_wrong_filename(self, tmp_path: Path):
        """Raise ValueError when the file doesn't match the expected filename."""
        bad_file = tmp_path / "components" / "training" / "my_comp" / "wrong.py"
        bad_file.parent.mkdir(parents=True)
        bad_file.write_text("")

        with pytest.raises(ValueError, match="Expected component.py"):
            _build_asset_dict_from_repo_path(tmp_path, "components", bad_file, "component.py")

    def test_raises_on_invalid_depth(self, tmp_path: Path):
        """Raise ValueError when path has neither 3 nor 4 parts."""
        # Too shallow: only 2 parts (category/filename)
        ...
        with pytest.raises(ValueError, match="Path must be"):
            _build_asset_dict_from_repo_path(tmp_path, "components", shallow_file, "component.py")
    ...
    def test_raises_on_too_deep_path(self, tmp_path: Path):
        ...
        with pytest.raises(ValueError, match="Path must be"):
            _build_asset_dict_from_repo_path(tmp_path, "components", deep_file, "component.py")
